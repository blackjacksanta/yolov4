...
#class_map = [blast_wound,
#blight_wound,
#narrowbrownspot_wound]
...
def draw_bounding_box(img, class_id, confidence, x, y, x_plus_w, y_plus_h):
    ...
    if iclass == 10:
       ...
       return all_t
    	...
    	if y < 30:
        	cv2.rectangle(img, (x,y), (x+250+ext_length,y+40), color, -1)
        	cv2.putText(img, label+" "+str((confidence))[:5]+"%", (x+5,y+30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 3)
    	else:
        	cv2.rectangle(img, (x,y-40), (x+250+ext_length,y), color, -1)
        	cv2.putText(img, label+" "+str((confidence))[:5]+"%", (x+5,y-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 3)
